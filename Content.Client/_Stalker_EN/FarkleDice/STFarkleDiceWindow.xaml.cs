...
public sealed partial class STFarkleDiceWindow : FancyWindow
{
    ...
    private static readonly List<ProtoId<GuideEntryPrototype>> HelpGuides = new() { "STFarkleDice" };
    ...
    public STFarkleDiceWindow()
    {
        ...
        Score2000.OnPressed += _ => OnSetTargetScore?.Invoke(2000);
        Score3000.OnPressed += _ => OnSetTargetScore?.Invoke(3000);
        ...
        SetTargetScoreButton.OnPressed += _ =>
        {
            if (int.TryParse(CustomTargetScoreInput.Text, out var targetScore))
            {
                targetScore = Math.Clamp(targetScore, 1000, 5000);
                OnSetTargetScore?.Invoke(targetScore);
                CustomTargetScoreInput.Text = "";
            }
        };
    }
    ...
    public void UpdateState(STFarkleDiceBoundUiState state)
    {
        ...
        TargetScoreLabel.Text = state.TargetScore > 0
            ? Loc.GetString("farkle-target-score", ("score", state.TargetScore))
            : Loc.GetString("farkle-no-target-score");
    ...
    private static string GetStatusMessage(STFarkleDiceBoundUiState state, bool isYourTurn)
    {
        ...
        {
            STFarkleDicePhase.WaitingForPlayers when state.TargetScore == 0
                => Loc.GetString("farkle-set-target-to-start"),
