...
using Content.Client._Stalker_EN.Shop;
...
public sealed partial class ShopMenu : DefaultWindow
{
    ...
    // stalker-changes-en: sell confirmation persistence
    private readonly IConfigurationManager _cfg;
    private readonly HashSet<string> _suppressedSellConfirmations = new();
    private STSellConfirmWindow? _sellConfirmWindow;
    ...
    public ShopMenu()
    {
        ...
        // stalker-changes-en: load sell confirmation suppressions from CVar
        _cfg = IoCManager.Resolve<IConfigurationManager>();
        var raw = _cfg.GetCVar(STCCVars.ShopSuppressedSellConfirmations);
    ...
    private void AddListingGui(ListingData listing, bool sell = false)
    {
        ...
        newListing.ShopItemButton.OnButtonDown += (args) =>
        {
            ...
            // stalker-changes-en: sell confirmation popup
            if (listing.ProductEntity != null
                && !_suppressedSellConfirmations.Contains(listing.ProductEntity))
            {
                if (_sellConfirmWindow != null && _sellConfirmWindow.IsOpen)
                {
                    _sellConfirmWindow.MoveToFront();
                    return;
                }

                var priceStr = listing.OriginalCost.Values.FirstOrDefault().ToString();
                _sellConfirmWindow = new STSellConfirmWindow(listingName, priceStr, _locMoneyName);
                _sellConfirmWindow.OnConfirmed += () =>
                {
                    if (_sellConfirmWindow.DontAskAgain && listing.ProductEntity != null)
                        SuppressSellConfirmation(listing.ProductEntity);

                    ProceedWithSell(args, listing, listingCount);
                };
                _sellConfirmWindow.OpenCentered();
                return;
            }
    ...
    // stalker-changes-en: extracted sell flow for reuse with confirmation popup
    private void ProceedWithSell(BaseButton.ButtonEventArgs args, ListingData listing, int listingCount)
    {
        if (listingCount <= 1)
        {
            OnListingButtonPressed?.Invoke(args, listing, true, _balance, listingCount);
            return;
        }
        ...
        {
            OnListingButtonPressed?.Invoke(args, listing, true, _balance, _shopSlider.GetSliderValue());
        };
    }

    // stalker-changes-en: persist sell confirmation suppression
    private void SuppressSellConfirmation(string prototypeId)
    {
        _suppressedSellConfirmations.Add(prototypeId);
