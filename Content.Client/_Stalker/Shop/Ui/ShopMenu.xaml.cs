...
public sealed partial class ShopMenu : DefaultWindow
{
    ...
    private void AddListingGui(ListingData listing, bool sell = false)
    {
        ...
        //stalker-en-start: add Compare button logic
        if (listing.ProductEntity != null)
        {
            var proto = _prototypeManager.Index<EntityPrototype>(listing.ProductEntity);
            var statsSystem = _entitySystem.GetEntitySystem<StatsExamineSystem>();

            if (proto.TryGetComponent<ArmorComponent>(out _, _componentFactory) && !proto.TryGetComponent<ZoneArtifactComponent>(out _, _componentFactory))
            {
                newListing.CompareStatsButton.Visible = true;
                newListing.CompareStatsButton.OnPressed += _ =>
                {
                    var player = _playerManager.LocalSession?.AttachedEntity;
                    if (player.HasValue)
                        statsSystem.OpenStatsWindowFromPrototype(player.Value, listing.ProductEntity.Value);
                };
            }
            else
            {
                newListing.CompareStatsButton.Visible = false;
            }
        }
        else
        {
