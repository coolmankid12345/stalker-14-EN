ent-ZoneArtifactSparkler = Sparkler
    .desc = Artifact
    .suffix = Stalker, Sparkler
ent-ZoneArtifactBattery = Battery
    .desc = Artifact
    .suffix = Stalker, Battery
ent-ZoneArtifactFlash = { ent-BaseArtifact }
    .desc = Artifact
    .suffix = Stalker, Flash
ent-ArtMoon = { ent-BaseArtifact }
    .desc = Artifact
    .suffix = Stalker, Moonlight
ent-ZoneArtifactTesla = { ent-BaseArtifact }
    .desc = Artifact
    .suffix = Stalker, Tesla
ent-ZoneArtifactIce = { ent-BaseArtifact }
    .desc = Artifact
    .suffix = Stalker, Snowflake
ent-ZoneArtifactAccumulator = Accumulator
    .desc = Artifact
    .suffix = Stalker, Accumulator
ent-ZoneArtifactDiamond = { ent-BaseArtifact }
    .desc = Artifact
    .suffix = Stalker, Diamond
ent-ZoneArtifactCharger = { ent-BaseArtifact }
    .desc = Artifact
    .suffix = Stalker, Charger
